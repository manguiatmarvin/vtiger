...
class Settings_ExtensionStore_ExtnStore_Connector {
    ...
    protected function initializeAuth() {
        ...
        if (!$this->auth) {
            ...
            if ($db->num_rows($authResult)) {
                ...
                if ($this->auth['deleted'] == 1)$this->auth = null;
            }
            if(empty($this->auth['password']))$this->auth['password'] = isset($_SESSION[$this->identifier_name . '_password']) ? $_SESSION[$this->identifier_name . '_password'] : null;
            if(empty($this->auth['username']))$this->auth['username'] = isset($_SESSION[$this->identifier_name . '_username']) ? $_SESSION[$this->identifier_name . '_username'] : null;
    ...
    protected function api($uri, $method, $params, $auth) {
        ...
        $fn = ($method == "GET" || $method == "DLD") ? "doGet" : "doPost";if ($method == "PUT")$fn = "doPut";
